...
```
postgres=#  select gp_execution_dbid(), datname, pg_size_pretty(pg_database_size(datname)) 
postgres-#  from gp_dist_random('pg_database') order by 2,1,pg_database_size(datname) desc;
...
```
postgres=#  select gp_execution_dbid(), datname, pg_size_pretty(pg_total_relation_size('表名')) 
postgres-#  from gp_dist_random('gp_id') ;
```
